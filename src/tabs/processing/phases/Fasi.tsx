...
import { addItemToCart, getCurrentOrderRestaurantName, updateTotal } from "../../../store/cartSlice";
import { Ionicons } from "@expo/vector-icons";
import { Switch, ScrollView, TextPropTypes } from "react-native";
...
function FaseCarboidrati({ navigation }: GenericNavProps<"Fasi">) {
  ...
  if (allIngredients) {
    ...
    plates = allIngredients.map((ingrediente, index) => {
        ...
        <Card style={genericCardMargin} key={index}>
          <Card.Content>
            ...
            <Card.Actions style={{flexDirection: "row-reverse"}}>
              {/**
               * Picker per le quantità
               * e ricalcolo dei valori in funzione delle quantità
               */}
               ...
               <Slider
                  ...
                  onValueChange={value => onToggleSlider(value, ingrediente.quantity, index)}
               />
              <Switch
                color="green"
...
const FasiStack = () => {
  return (
    <Stack.Navigator initialRouteName="Fasi">
      <Stack.Screen
        name="Fasi"
        options={{ headerTitle: "Carboidrati", headerShown: false }}
